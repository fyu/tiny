...
static int mask_nonmaxima_in_search_grid = 0;
...
// static RNScalar min_points_per_square_meter = 100;
...
static RNLength min_rotation_tolerance = RN_PI / 4.0;
...
CreateDensityGrid(R3SurfelScene *scene, const R2Grid& zsupport_grid)
{
  ...
  for (int i = 0; i < tree->NNodes(); i++) {
    ...
    for (int j = 0; j < node->NBlocks(); j++) {
      ...
      for (int j = 0; j < block->NSurfels(); j++) {
        ...
        // Get coordinates
        double x = origin.X() + surfel->X();
        double y = origin.Y() + surfel->Y();

        // Check height
        double z = origin.Z() + surfel->Z();
        if ((min_height != RN_UNKNOWN) || (max_height != RN_UNKNOWN)) {
          RNScalar zsupport = zsupport_grid.WorldValue(x, y);
        ...
        // Add point
        grid->RasterizeWorldPoint(x, y, 1.0);
      }
  ...
  // Mask sparse areas
  RNScalar scale = grid->WorldToGridScaleFactor();
...
CreateSearchGrid(R3SurfelScene *scene, const R2Grid& zsupport_grid)
{
  ...
  // Mask non-maxima
  if (mask_nonmaxima_in_search_grid) grid->MaskNonMaxima(1);
...
CreateDetectionForSurfelObject(ObjectParse *parse, R3SurfelScene *scene, R3SurfelObject *object, const R2Grid& zsupport_grid)
{
  ...
  ObjectPointSet pointset;
  LoadSurfels(pointset, scene, surfels, max_points);
  if (pointset.NPoints() < min_points) { delete surfels; return 0; }
  if (pointset.NPoints() == 0) { delete surfels; return 0; }
...
static int
RemoveWeakAssignments(ObjectParse *parse, RNScalar min_score)
{
  // Check min score
  if (min_score <= 0) return 1;
...
int main(int argc, char **argv)
{
  ...
  if (remove_weak_assignments) {
    if (!RemoveWeakAssignments(parse, min_score)) exit(-1);
  }

  // Remove equivalent assignments
  if (remove_equivalent_assignments) {
    if (!Sort(parse)) exit(-1);
    if (!RemoveEquivalentAssignments(parse, min_assignment_spacing, min_rotation_tolerance)) exit(-1);
  }

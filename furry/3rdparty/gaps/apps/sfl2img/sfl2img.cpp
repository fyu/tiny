...
WriteColorGrids(R3SurfelScene *scene, const char *directory_name)
{
  ...
  R2Grid red_grid(count_grid);
  R2Grid green_grid(count_grid);
  R2Grid blue_grid(count_grid);
    ...
    if (node->NParts() > 0) { // (node->NBlocks() == 0) || (node->Resolution() < 10 / pixel_spacing)) {
      ...
      for (int i = 0; i < node->NBlocks(); i++) {
        ...
        for (int j = 0; j < block->NSurfels(); j++) {
          ...
          R2Point grid_position = zmax_grid.GridPosition(R2Point(x, y));
          ...
          if ((zmin == R2_GRID_UNKNOWN_VALUE) || (z < zmin)) {
            zmin_grid.SetGridValue(ix, iy, z);
          }
 
          // Update zmax grid (and rgb)
          RNScalar zmax = zmax_grid.GridValue(ix, iy);
          if ((zmax == R2_GRID_UNKNOWN_VALUE) || (z > zmax)) {
            zmax_grid.SetGridValue(ix, iy, z);
  ...
  // Write grids
  if (!WriteGrid(count_grid, directory_name, "Color", "Count")) return 0;
  if (!WriteGrid(zmin_grid, directory_name, "Color", "ZMin")) return 0;
  if (!WriteGrid(zmax_grid, directory_name, "Color", "ZMax")) return 0;
...
ParseArgs(int argc, char **argv)
{
  ...
  if (default_grids) write_color_grids = 1;

...
static int 
MinimizeCERES(const RNSystemOfEquations *system, RNScalar *io)
{
...
static void SPLMFunction(double *x, double *y, int n, int m, void *data)
{
...
static void SPLMJacobian(double *x, struct splm_ccsm *jac, int n, int m, void *data)
{
...
static int 
MinimizeSPLM(const RNSystemOfEquations *system, RNScalar *io)
{
  // Get convenient variables
...
static int 
MinimizeMINPACK(const RNSystemOfEquations *system, RNScalar *io)
{
  // Get convenient variables
  const int m = system->NEquations();
  const int n = system->NVariables();
...
static int 
MinimizeCSPARSE(const RNSystemOfEquations *system, RNScalar *io)
{
#if 1
  // Get convenient variables
  const int n = system->NVariables();
  ...
  for (int i = 0; i < system->NEquations(); i++) {
    RNEquation *equation = system->Equation(i);
    RNPolynomial *polynomial = equation->Polynomial();
    if (!polynomial) {
      cs_spfree(a); delete [] b; delete [] x; delete [] lhs;
      fprintf(stderr, "Unable to minimize algebraic system of equations with CSparse\n");
      return 0;
    }

    // Initialize data for row 
    for (int i = 0; i < n; i++) lhs[i] = 0;
    double rhs = 0;
    ...
    for (int j = 0; j < polynomial->NTerms(); j++) {
      ...
      if (term->NVariables() == 0) {
        ...
        if (best_partial_derivative != 0) {
          ...
          for (int k = 0; k < term->NVariables(); k++) {
            if (k == best_variable) continue; // (assumes variable is linear)
          ...
          // Add term to row
          if (factor != 0) lhs[best_variable] += factor;
        }
      }
    }

    // Count number of nonzero entries in row
    int nz = 0;
    for (int i = 0; i < n; i++) {
      if (lhs[i] == 0) continue;
      nz++;
    ...
    // Check number of nonzero entries in row
    if (nz > 0) {
      assert(m < mm);
      for (int i = 0; i < n; i++) {
        if (lhs[i] == 0) continue;
        cs_entry(a, m, i, lhs[i]);
      }
      b[m] = rhs;
      m++;
    }
  ...
  return status;
#else
  RNAbort("CSPARSE solver not implemented");
  return 0;
#endif
}
...
{
  // Check solver
  if (solver == RN_SPLM_SOLVER) return MinimizeSPLM(this, x);
  else if (solver == RN_MINPACK_SOLVER) return MinimizeMINPACK(this, x);
  else if (solver == RN_CERES_SOLVER) return MinimizeCERES(this, x);
  else if (solver == RN_CSPARSE_SOLVER) return MinimizeCSPARSE(this, x);

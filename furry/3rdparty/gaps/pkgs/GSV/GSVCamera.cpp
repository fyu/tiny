...
R2Point GSVCamera::
DistortedPositionFromAngles(RNAngle x_dir, RNAngle y_dir) const
{
  // (xdir, y_dir) is undistorted position given in angular rotations
  ...
  fprintf(stderr, "Unrecognized distortion type: %d\n", distortion_type);
  return R2zero_point;
}
...
DistortedPosition(const R2Point& undistorted_image_position) const
{
  ...
  RNAngle dir_x = atan( delta_x / focal_x );
  RNAngle dir_y = atan( delta_y / focal_y );
  return DistortedPositionFromAngles(dir_x, dir_y);
}

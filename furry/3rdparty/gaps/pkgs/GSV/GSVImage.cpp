...
DistortedImage(void) const
{
  ...
  sprintf(distorted_image_name, "%s/%s/segment_%02d/unstitched_%06d_%02d.jpg", 
    scene->RawDataDirectoryName(), run_name, segment_index, panorama_index, image_index);
  ...
  if (!distorted_image->Read(distorted_image_name)) {
    fprintf(stderr, "Unable to read distorted image %s\n", distorted_image_name);
...
UndistortedImage(void) const
{
  ...
  if (RNFileExists(undistorted_image_name)) { 
    ...
    if (!undistorted_image->Read(undistorted_image_name)) {
      fprintf(stderr, "Unable to read undistorted image %s\n", undistorted_image_name);
    ...
    if (!camera->UndistortImage(*distorted_image, *undistorted_image)) {
      fprintf(stderr, "Unable to undistort image for %s\n", undistorted_image_name);
      delete undistorted_image;
      return NULL;
    }
    ...
    if (scene->CreateCacheDataDirectory("undistorted_images", run)) {
      ...
      if (!undistorted_image->Write(undistorted_image_name)) {
        fprintf(stderr, "Unable to write undistorted image for %s\n", undistorted_image_name);
      }
    }
...
UndistortedPosition(const R3Point& world_position, int column_index) const
{
  ...
  GSVCamera *camera = Camera();
  if (!camera) return 0;
  RNCoord xcenter = 0.5 * Width();
  RNCoord ycenter = 0.5 * Height();
  RNAngle xfov = 0.5 * XFov();
  RNAngle yfov = 0.5 * YFov();
  int width = Width();
  int height = Height();
  ...
  RNCoord x = xcenter + 0.5 * width * camera_position.X() / -(camera_position.Z() * tan(xfov));
  RNCoord y = ycenter + 0.5 * height * camera_position.Y() / -(camera_position.Z() * tan(yfov));
...
UndistortedPosition(const R3Point& world_position) const
{
  ...
  for (int i = 0; i < Width()/2; i++) {
    ...
    // Compute coordinates of projection
    x = xcenter + 0.5 * width * camera_position.X() / -(camera_position.Z() * tan(xfov));
    y = ycenter + 0.5 * height * camera_position.Y() / -(camera_position.Z() * tan(yfov));

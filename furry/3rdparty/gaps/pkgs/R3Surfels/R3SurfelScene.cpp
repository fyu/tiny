 ...
 // COPY FEATURES
  ...
  for (int i = 0; i < scene2.NObjectRelationships(); i++) {
    ...
    if (relationship2->NOperands() > 0) {
      operands1 = new RNScalar [ relationship2->NOperands() ];
      for (int j = 0; j < relationship2->NOperands(); j++) {
        RNScalar operand2 = relationship2->Operand(j);
        operands1[j] = operand2;
      }
    }
    R3SurfelObjectRelationship *relationship1 = new R3SurfelObjectRelationship(type, objects1, operands1);
...
ReadAsciiFile(const char *filename)
{
  ...
  int nassignments, dummy;
  ReadAsciiName(fp, buffer);
  if (strcmp(buffer, "None")) SetName(buffer);
  fscanf(fp, "%d%d%d%d%d%d%d", &nnodes, &nobjects, &nlabels, &nfeatures, &nobject_relationships, &nlabel_relationships, &nassignments);
  ...
  for (int i = 0; i < nobject_relationships; i++) {
    ...
    R3SurfelObjectRelationship *relationship = new R3SurfelObjectRelationship(type, objs, operands);
  ...
  for (int i = 0; i < nlabel_relationships; i++) {
    ...
    R3SurfelLabelRelationship *relationship = new R3SurfelLabelRelationship(type, objs, operands);

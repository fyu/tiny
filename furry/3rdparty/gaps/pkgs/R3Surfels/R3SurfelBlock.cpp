...
HasAerial(void) const
{
  ...
  if (!flags[R3_SURFEL_BLOCK_SCANNER_SOURCES_UPTODATE_FLAG]) {
    R3SurfelBlock *block = (R3SurfelBlock *) this;
    block->UpdateScannerSources();
  }
...
UpdateResolution(void)
{
  ...
  if (nsurfels > 0) {
    ...
    RNLength total_squared_distance = 0;
    for (int i = 0; i < nsamples; i++) {
      R3Surfel *surfel1 = &surfels[i*step];
      float x1 = surfel1->X();
      float y1 = surfel1->Y();
      float z1 = surfel1->Z();
      RNLength min_squared_distance = FLT_MAX;
      for (int j = 0; j < nsurfels; j++) {
        R3Surfel *surfel2 = &surfels[j];
        float x2 = surfel2->X();
        float y2 = surfel2->Y();
        float z2 = surfel2->Z();
        float dx = x1 - x2;
        ...
        if ((squared_distance < min_squared_distance) && (i != j)) {
          min_squared_distance = squared_distance;
        }
      }
      total_squared_distance += min_squared_distance;
    }

    // Resolution is samples / unit_length
    if (total_squared_distance == 0) resolution = RN_INFINITY;
    else resolution = sqrt(nsamples / total_squared_distance);
  }
...
UpdateScannerSources(void)
{
  ...
  // Mark flags uptodate
  flags.Add(R3_SURFEL_BLOCK_SCANNER_SOURCES_UPTODATE_FLAG);
}

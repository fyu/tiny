...
#define R3_SURFEL_OBJECT_SCANNER_SOURCES_UPTODATE_FLAG  0x0001

#define R3_SURFEL_OBJECT_HAS_AERIAL_FLAG                0x0010
#define R3_SURFEL_OBJECT_HAS_TERRESTRIAL_FLAG           0x0020
...
{
  // Return resolution of node (spacing between surfels)
...
HasAerial(void) const
{
  ...
  if (!flags[R3_SURFEL_OBJECT_SCANNER_SOURCES_UPTODATE_FLAG])
    ((R3SurfelNode *) this)->UpdateScannerSources();
  return flags[R3_SURFEL_OBJECT_HAS_AERIAL_FLAG];
}
...
HasTerrestrial(void) const
{
  ...
  if (!flags[R3_SURFEL_OBJECT_SCANNER_SOURCES_UPTODATE_FLAG]) 
    ((R3SurfelNode *) this)->UpdateScannerSources();
  return flags[R3_SURFEL_OBJECT_HAS_TERRESTRIAL_FLAG];
}
...
UpdateResolution(void)
{
  ...
  // Average resolution of blocks
  if (Complexity() > 0) {
#if 0
    // Compute sum and weight
    RNLength sum  = 0;
    RNScalar weight = 0;
    for (int i = 0; i < NBlocks(); i++) {
      R3SurfelBlock *block = Block(i);
      sum += block->NSurfels() * block->Resolution();
      weight += block->NSurfels();
    }

    // Compute weighted average
    if (weight > 0) resolution = sum / weight;
#else
...
void R3SurfelNode::
UpdateScannerSources(void)
{
  // Reset flags
  flags.Remove(R3_SURFEL_OBJECT_HAS_AERIAL_FLAG);
  flags.Remove(R3_SURFEL_OBJECT_HAS_TERRESTRIAL_FLAG);

  // Update flags
  for (int i = 0; i < NBlocks(); i++) {
    R3SurfelBlock *block = Block(i);
    if (block->HasAerial()) flags.Add(R3_SURFEL_OBJECT_HAS_AERIAL_FLAG);
    if (block->HasTerrestrial()) flags.Add(R3_SURFEL_OBJECT_HAS_TERRESTRIAL_FLAG);
  }

  // Mark flags uptodate
  flags.Add(R3_SURFEL_OBJECT_SCANNER_SOURCES_UPTODATE_FLAG);
}

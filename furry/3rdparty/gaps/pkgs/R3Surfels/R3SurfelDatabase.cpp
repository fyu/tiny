...
InternalReadBlock(R3SurfelBlock *block)
{
  ...
  if (!ReadSurfel(fp, block->surfels, block->nsurfels, swap_endian)) return 0;
...
InternalSyncBlock(R3SurfelBlock *block)
{
  ...
  if (!WriteSurfel(fp, block->surfels, block->nsurfels, swap_endian)) return 0;
...
OpenFile(const char *filename, const char *rwaccess)
{
  ...
  if (!strcmp(this->rwaccess, "w+b")) {
    ...
    if ((major_version != 1) || (minor_version != 17)) {
